...
class PublicJobAPIAdapter(BaseJobSource):
    ...
    def __init__(self, base_url: str = "https://example.invalid/jobs"):
        super().__init__()
    ...
    def fetch_jobs(self, search_config: SearchCriteria | dict) -> list[JobSchema]:
        criteria = search_config if isinstance(search_config, SearchCriteria) else SearchCriteria(roles=search_config.get("JOB_ROLES", []), locations=search_config.get("LOCATIONS", []))
        jobs: list[JobSchema] = []
